...
function packTravel(topic: string): Pack {
  ...
  return {
    experts: [
      {
        name: names[0]!,
        description: "항목별 예산 · 가성비 실무",
        bio: "교통·숙소·식비·이동을 한 줄씩 나눠 ‘남는지·터지는지’부터 봅니다.",
        iconName: "Wallet",
        color: "emerald",
      },
      {
        name: names[1]!,
        description: "경험 밀도 · 지금 쓰기 쪽",
        bio: "절약만 하다 놓치는 한 끼·한 밤을 짚되, 설교나 스님 말투는 안 씁니다.",
        iconName: "Zap",
        color: "amber",
      },
      {
        name: names[2]!,
        description: "뉴스·커뮤 · 물가 체감",
        bio: "기사랑 커뮤에 도는 말을 근거 삼되, 숫자는 직접 검색해 보라고 못 박습니다.",
        iconName: "Newspaper",
        color: "cyan",
      },
    ],
    deep1: `**총액만 보지 말고 항목으로 쪼개** 봐요. 왕복 교통·숙박 며칠·1일 식비·현지 이동(버스 vs 렌트+유류)·비상금까지 한 표에. 질문에 숫자가 있으면 그걸 기준으로 **1인당·1일당**으로 나눠 보면 바로 허점이 보여요.

숙소만 세고 밥값을 ‘대충’ 두면 **막상 현지에서 예산 터지는** 패턴이 제일 많아요. 렌터카 넣을 거면 주차·유류를 **같은 줄에 붙이라**고 주장해요.`,
    rebuttal2: `표만 잘 짜도 **여행 온 맛**(해식 한 번, 뷰 나오는 숙소 한 밤, 일부러 비운 저녁)이 다 빠질 수 있어요. 무조건 호사부리자는 게 아니라 **한 덩어리는 울컥하게 쓰자**는 쪽이에요—나중에 같은 체력·일정으로 못 올 수도 있으니까요.

다만 불교 격언이나 스님 말투로 인생 타령하는 건 여행 결정이랑 안 어울려요. 친구랑 수다하듯이 **뭐에 돈 쓰면 후회가 덜한지**만 말할게요.`,
    critique3: `둘 다 맞는 말만 했어요. 저는 **뉴스랑 커뮤니티에 떠도는 체감**을 같이 보라고 해요. 제주·성수기·렌트 값만 쳐도 ‘쎄다/괜찮다’ 말이 갈려요—**정답은 출발 며칠 전에 직접 검색**해서 자기 일정에 맞춰 보는 거고요.

가짜 기사 제목이나 퍼센트로 설득하는 건 안 할게요. 대신 **성수기 주말이면 숙박이 튀는지**, 항공·배는 예약 조건이 빡센지 **한 번만 확인**하라고요.`,
    free1: `인스타·SNS용이면 솔직히 말해. 그럼 포기할 코스부터 달라져도 돼.`,
    free2: `총예산 한 숫자부터 적어. 없으면 무조건 싸움 나요.`,
    free3: `물가 체감은 출발 3~5일 전에 키워드 세 개만 다시 쳐 봐. 그때 숫자가 진짜예요.`,
    conclusion: `정리하면 **총액 안에서 항목표를 먼저 쓰고**, 그다음 **꼭 쓰고 싶은 체험 한 덩어리**만 밀어 붙인 뒤, **뉴스·커가 말하는 물가는 검색으로 교차 확인**하자는 쪽으로 합의했습니다.`,
    consensus: {
      summary: `가성비·지금 경험·정보 확인 세 축이 만났고, **항목별 시트 + 우선 체험 한 줄 + 출발 전 물가 재확인**으로 가자는 데 수렴했습니다.`,
      actions: [
        "왕복 교통·숙소·식비·이동(렌트면 유류까지)·비상금을 표 한 장에 숫자로 적기",
        "꼭 하고 싶은 체험 1개만 골라 시간·돈 먼저 배정하기",
        "출발 3~5일 전에 숙소·렌트·식비 키워드로 뉴스·커뮤 검색해 체감 범위 잡기",
        "±1일 또는 대체 코스 1개로 플랜 B 적어 두기",
      ],
    },
  };
}
